...
local quest_name_to_stop_at = "A Wild Rose by Any Other Name"
...
local chars = {
    ...
    --    "Grillbrick Taylor@Zalera",
    --    "Grillbrick Taylor@Omega",
    "Cepandrius Hoid@Excalibur",
    "Anna Hessin@Leviathan",
    "Samuel Mrtwister@Leviathan",
    "Samuueill Daddy@Malboro",
    "Sasel Samufras@Lich",
}
...
local qst_reloader_threshold = 15 -- this is how many loops quest reloader will wait before it triggers a reload if it finds you being stuck,
-- set this higher if you end up having issues with follow quests and similar
...
local function SquaredDistance(x1, y1, z1, x2, y2, z2)
    ...
    if success then
        ...
        LogInfo("[QSTC] Failed to square distance " ..
        x1 .. " " .. y1 .. " " .. z1 .. " against " .. x2 .. " " .. y2 .. " " .. z2)
        return nil
    end
end
        ...
        if IsAddonReady("SelectYesno") or IsAddonReady("DifficultySelectYesNo") then
            ...
            if string.find(text1, "Commence") or string.find(text2, "Commence") then
                if string.find(text1, "Commence") then
                    repeat
                        LogInfo(
                        "[QSTC] Instance helper: Commence under SelectYesno found, attempting to start the instance")
                ...
                elseif string.find(text2, "Commence") then
                    repeat
                        LogInfo(
                        "[QSTC] Instance helper: Commence under DifficultySelectYesNo found, attempting to start the instance")
